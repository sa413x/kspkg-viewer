...
TextEditor::TextEditor()
    : mLineSpacing( 1.0f ), mUndoIndex( 0 ), mTabSize( 4 ), mOverwrite( false ), mReadOnly( false ), mWithinRender( false ),
      ...
      mLeftMargin( 10 ), mCursorPositionChanged( false ), mColorRangeMin( 0 ), mColorRangeMax( 0 ), mSelectionMode( SelectionMode::Normal ),
      mCheckComments( true ), mLastClick( -1.0f ), mHandleKeyboardInputs( true ), mHandleMouseInputs( true ), mIgnoreImGuiChild( false ),
    ...
    if ( lineNo >= 0 && lineNo < ( int )mLines.size() ) {
        ...
        while ( ( size_t )columnIndex < line.size() ) {
            ...
            if ( line[ columnIndex ].mChar == '\t' ) {
                ...
                float oldX = columnX;
                float newColumnX =
                    ( 1.0f + std::floor( ( 1.0f + columnX ) / ( float( mTabSize ) * spaceSize ) ) ) * ( float( mTabSize ) * spaceSize );
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HLSL() {
    ...
    if ( !inited ) {
        ...
        langDef.mTokenRegexStrings.push_back(
            std::make_pair< std::string, PaletteIndex >( R"(L?\"(\\.|[^\"])*\")", PaletteIndex::String ) );
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::AngelScript() {
    ...
    if ( !inited ) {
        static const char* const keywords[] = {
            "and",      "abstract", "auto",      "bool",   "break",  "case",      "cast",  "class",  "const",     "continue",
            "default",  "do",       "double",    "else",   "enum",   "false",     "final", "float",  "for",       "from",
            "funcdef",  "function", "get",       "if",     "import", "in",        "inout", "int",    "interface", "int8",
            "int16",    "int32",    "int64",     "is",     "mixin",  "namespace", "not",   "null",   "or",        "out",
            "override", "private",  "protected", "return", "set",    "shared",    "super", "switch", "this ",     "true",
            "typedef",  "uint",     "uint8",     "uint16", "uint32", "uint64",    "void",  "while",  "xor" };
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HTML() {
    ...
    if ( !inited ) {
        langDef.mTokenRegexStrings.push_back(
            std::make_pair< std::string, PaletteIndex >( "<!DOCTYPE.*?>", PaletteIndex::PreprocIdentifier ) );
        langDef.mTokenRegexStrings.push_back( std::make_pair< std::string, PaletteIndex >( R"(\"(\\.|[^\"])*\")", PaletteIndex::String ) );
        langDef.mTokenRegexStrings.push_back( std::make_pair< std::string, PaletteIndex >( R"(\'(\\.|[^\'])*\')", PaletteIndex::String ) );
        langDef.mTokenRegexStrings.push_back( std::make_pair< std::string, PaletteIndex >( R"(<[a-zA-Z/-]*>)", PaletteIndex::Keyword ) );
        langDef.mTokenRegexStrings.push_back( std::make_pair< std::string, PaletteIndex >( R"(<[a-zA-Z/-]* )", PaletteIndex::Keyword ) );

        langDef.mTokenRegexStrings.push_back(
            std::make_pair< std::string, PaletteIndex >( R"([a-zA-Z_-][a-zA-Z0-9_-]*=)", PaletteIndex::KnownIdentifier ) );
        langDef.mTokenRegexStrings.push_back(
            std::make_pair< std::string, PaletteIndex >( R"([a-zA-Z_-][a-zA-Z0-9_-]*>)", PaletteIndex::KnownIdentifier ) );
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JS() {
    ...
    static LanguageDefinition langDef;
    if ( !inited ) {
        static const char* const keywords[] = { "break",    "case",   "catch",  "class",  "const",      "continue", "debugger", "default",
